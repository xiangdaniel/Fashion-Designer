...
def fashion_image(path, input_width, input_height):
...
def modify_mask(image, mask, pattern):
...
def display_instances(image, boxes, masks, class_ids, class_names, fashion_name,
                      scores=None, title="",
    ...
    # path to DCGAN
    ROOT_DIR = os.getcwd()
    IMAGE_DIR = os.path.join(ROOT_DIR, "results_fashion")
    ...
    for i in range(N):
        if class_names[class_ids[i]] != 'handbag' and class_names[class_ids[i]] != 'tie' and class_names[class_ids[i]] != 'cell phone' and class_names[class_ids[i]] != 'umbrella':
            continue

        color = colors[i]

        # TODO: Generate patterns randomly
        #file_names = next(os.walk(IMAGE_DIR))[2]
        #dcgan_image = fashion_image(os.path.join(IMAGE_DIR, random.choice(file_names)), masked_image.shape[1], masked_image.shape[0])
        dcgan_image = fashion_image(fashion_name, masked_image.shape[1], masked_image.shape[0])
        #skimage.io.imsave('sample.png', dcgan_image)
        ...
        # Mask
        mask = masks[:, :, i]
        masked_image = modify_mask(masked_image, mask, dcgan_image)
